...
## Round-5（重新部署 + prompt 规则同步 + 编码列描述注入后）

| 指标 | R3（AND/HAVING 修复后） | **R5（本轮）** |
|---|---|---|
| 尝试执行 | 6/30 | 6/30 |
| 正确 | 1（Q20） | **1（Q2——新增得分题）** |
| 耗时 | 38s | 41s |

关键变化：**得分题从 Q20 换成了 Q2**——prompt 规则同步（维度-only 合法 +
编码列警告）让 Q2（邮编列表，纯维度题）首次答对且 5 行与金标吻合；但
Q20 本轮反而答错，Q0 依旧选聚合度量并撞 NULL 排序。

**R5 核心结论：单发映射管道的得分集在轮次间漂移（Q20↔Q2），attempted
稳定在 6-9/30、准确率 ~3%。** 稳定性问题的根源是映射层对措辞/上下文高度
敏感且无校验重试——这正是需要 agentic 循环（生成→执行→验证→修正）而非
单发管道的实证。改进方向优先级调整为：
1. E1 成员名对齐器（吸收 R2b/R4 的命名错位）；
2. 映射后自校验：执行行数==0 或含 NULL 排名行时自动触发修正（对应
   chat2agent quality_gate 的空结果重试策略）；
3. 多采样投票（同题 3 次映射取多数）抵消单发方差。

## Round-6：同题 agentic 复跑（稳定性对照实验，2026-08-28）

GLM-5.3-Flash，当前分支代码（含 SQL 回收），30 题 3 并发：
完成 **30/30（100%）**，回收兜底命中 18 次，耗时 332s，
准确率 **11/30 = 36.7%**。
...
| 路径 | 完成 | 正确 | 准确率 | 轮次间稳定性 |
|---|---|---|---|---|
| **agentic 循环（GLM，两轮）** | 27→**30**/30 | 10/27, 11/30 | 37.0% → **36.7%** | ✅ ±0.3pp |
| **cube 单发映射（Qwen，三轮）** | 4→9→6/30 | 0,0,1 | 0%→0%→3.3% | ❌ 得分集漂移（Q20↔Q2） |

### 最终架构结论

1. **稳定性是两种架构的本质差异**：agentic 轮次间准确率波动 ±0.3pp；
   单发映射的得分集在轮次间整体漂移（上轮对的这轮错）——无校验无重试
   的管道对上下文扰动没有任何防御。
2. **准确率差一个数量级**：36.7% vs 3.3%（同为 30 题、同为 GLM/Qwen 级
   模型）。agentic 的优势来自 describe→execute→reflect 的自我修正循环。
3. **语义层的正确姿势是嵌入 agentic 而非替代它**：Cube 引擎作为 agentic
   循环中"指标形状问题"的加速器（命中时 20 倍延迟优势保持），失败时回落
   自由 SQL + 反思重试。
4. 回收兜底持续必要：本轮仍命中 18 次。